...
using System;
    ...
    public class ChatClient : IPhotonPeerListener
    {
        ...
        private const int DefaultMaxSubscribers = 100;
        ...
        private void HandleSubscribeEvent(EventData eventData)
        {
            ...
            #region NEW CHAT EXTENDED FEATURES
            if (eventData.Parameters.ContainsKey(ChatParameterCode.ChannelSubscribers) && channelsInResponse.Length == 1)
            {
                if (results[0])
                {
                    string channelName = channelsInResponse[0];
                    Dictionary<object, object> channelProperties =
                        eventData.Parameters[ChatParameterCode.ChannelProperties] as Dictionary<object, object>;
                    string[] subscribers = eventData.Parameters[ChatParameterCode.ChannelSubscribers] as string[];
                    if (channelProperties != null && channelProperties.Count > 0 && !this.PublicChannels.ContainsKey(channelName))
                    {
                        ChatChannel channel = new ChatChannel(channelName);
                        channel.MessageLimit = this.MessageLimit;
                        channel.SetProperties(channelProperties);
                        channel.Subscribers.AddRange(subscribers);
                        channel.Subscribers.Add(UserId);
                        this.PublicChannels.Add(channel.Name, channel);
                        this.listener.OnSubscribed(channelName, subscribers, channel.PublishSubscribers, channel.MaxSubscribers);
                        return;
                    }
                }
            }
        ...
        public bool Subscribe(string channel, int messagesFromHistory, bool publishSubscribers,
            int maxSubscribers = 100)
        {
            ...
            if (maxSubscribers != 0)
            {
                if (maxSubscribers < 1)
                {
                    this.listener.DebugReturn(DebugLevel.ERROR, "Cannot set MaxSubscribers < 1.");
                    return false;
                }
            ...
            {
                opParameters.Add(ChatParameterCode.ChannelProperties, properties);
            }
            return this.chatPeer.OpCustom(ChatOperationCode.Subscribe, opParameters, true);
        }

        private void HandleUserUnsubcribedEvent(EventData eventData)
        {
            ...
            {
                this.listener.DebugReturn(DebugLevel.WARNING, string.Format("Channel {0} does not contain unsusbcribed user {1}.", channelName, userId));
                return;
            }
        ...
        private void HandleUserSubscribedEvent(EventData eventData)
        {
            ...
            {
                this.listener.DebugReturn(DebugLevel.WARNING, string.Format("Channel {0} already contains newly susbcribed user {1}.", channelName, userId));
                return;
            }
            channel.Subscribers.Add(userId);
