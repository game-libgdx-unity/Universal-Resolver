...
namespace UTJ
{
    public struct RigidbodyTransform
    {
        ...
        {
            transform.init(ref position, ref rotation);
        ...
        {
            var force = transform.rotation * new Vector3(x, 0f, 0f);
            addForce(ref force);
        }

        public void addRelativeForceXY(float x, float y)
        {
            var force = transform.rotation * new Vector3(x, y, 0f);
            addForce(ref force);
        }

        public void addRelativeForceZ(float z)
        {
            var force = transform.rotation * new Vector3(0f, 0f, z);
//		var force = new Vector3(0f, 0f, z);
            addForce(ref force);
        }

        public void addRelativeForce(ref Vector3 power)
        {
            var force = transform.rotation * power;
            addForce(ref force);
        }
        ...
        {
            var t = transform.rotation * new Vector3(x, y, 0);
            r_acceleration_ += t;
        }

        public void addRelativeTorqueX(float torque)
        {
            var t = transform.rotation * new Vector3(torque, 0f, 0f);
            r_acceleration_ += t;
        }

        public void addRelativeTorqueZ(float torque)
        {
            var t = transform.rotation * new Vector3(0f, 0f, torque);
            r_acceleration_ += t;
        }

        public void solveForGround(float ground, float dt)
        {
            float predicted_y = transform.position.y + ((velocity_.y + (acceleration_.y * dt)) * dt);
            if (predicted_y > ground)
                return;
            velocity_.y = (ground - transform.position.y) / dt;
            acceleration_.y = 0f;
        }
        ...
        public void update(float dt)
        {
            ...
            // update position
            transform.position.x += velocity_.x * dt;
            transform.position.y += velocity_.y * dt;
            transform.position.z += velocity_.z * dt;
            ...
            // normalize
            var v2 = transform.rotation.x * transform.rotation.x;
            v2 += transform.rotation.y * transform.rotation.y;
            v2 += transform.rotation.z * transform.rotation.z;
            v2 += transform.rotation.w * transform.rotation.w;
            if (v2 == 0)
            {
            ...
            else
            {
                ...
                transform.rotation.x *= inv;
                transform.rotation.y *= inv;
                transform.rotation.z *= inv;
                transform.rotation.w *= inv;
            }
        }

        public void addSpringForce(ref Vector3 target, float ratio)
        {
            addForceX((target.x - transform.position.x) * ratio);
            addForceY((target.y - transform.position.y) * ratio);
            addForceZ((target.z - transform.position.z) * ratio);
        }

        public void addSpringForceX(float target_x, float ratio)
        {
            addForceX((target_x - transform.position.x) * ratio);
        }

        public void addSpringForceY(float target_y, float ratio)
        {
            addForceY((target_y - transform.position.y) * ratio);
        }

        public void addSpringForceZ(float target_z, float ratio)
        {
            addForceZ((target_z - transform.position.z) * ratio);
        }
        ...
        {
            var forward = transform.rotation * new Vector3(0f, 0f, 1f);
        ...
        {
            var left = transform.transformVector(ref CV.Vector3Left);
            var hori_left = new Vector3(left.x, 0f, left.z).normalized;
            var tq0 = Vector3.Cross(left, hori_left) * torque_level;
            addTorque(ref tq0);
            var forward = transform.transformVector(ref CV.Vector3Forward);
        ...
        public void addSpringTorque(ref Quaternion target, float torque_level)
        {
             ...
             * q.w = cos(theta/2)
             */
            var q = target * Utility.Inverse(ref transform.rotation);
            if (q.w < 0f)
            {
        ...
        {
            var tmp_x = transform.position.x;
            var tmp_y = transform.position.y;
            var tmp_z = transform.position.z;
            var diff_x = tmp_x - velocity_.x;
            var diff_y = tmp_y - velocity_.y;
            var diff_z = tmp_z - velocity_.z;
            var dt2 = dt * dt;
            transform.position.x += diff_x * damper + acceleration_.x * dt2;
            transform.position.y += diff_y * damper + acceleration_.y * dt2;
            transform.position.z += diff_z * damper + acceleration_.z * dt2;
            acceleration_ = CV.Vector3Zero;
            velocity_.x = tmp_x;
            velocity_.y = tmp_y;
            velocity_.z = tmp_z;
        ...
        public void restrictPositionVerlet(ref RigidbodyTransform grand_parent,
            ...
            float max_degree)
        {
            var diff = parent.transform.position - transform.position;

            // restriction for angle
            var rot_i = Utility.Inverse(ref grand_parent.transform.rotation);
            ...
            if (local_diff.z < restrict)
            {
                ...
                var diff0 = grand_parent.transform.rotation * local_diff;
                var new_pos = parent.transform.position - diff0;
            ...
            if (len > 0f)
            {
                diff *= (1f - length / len);
                transform.position += diff * 0.5f;
                parent.transform.position -= diff * 0.5f;
            }
        }

        public void solveRotationVerlet(ref RigidbodyTransform parent, ref RigidbodyTransform child)
        {
            var diff = transform.position - child.transform.position;
            if (diff.sqrMagnitude > 0f)
            {
                var up = parent.transform.rotation * CV.Vector3Up;
                transform.rotation = Quaternion.LookRotation(diff, up);
            }
        }
    }
} // namespace UTJ {
