...
namespace UnityIoC.Editor
{
    ...
    public class InjectIntoBindingSettingInspector : BaseInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var bindingSetting = (InjectIntoBindingSetting) target;
            
            bindingSetting.assemblyHolder = EditorGUILayout.ObjectField("Assembly definition", bindingSetting.assemblyHolder,
                typeof(AssemblyDefinitionAsset),
                false,
                GUILayout.MaxWidth(80),
                GUILayout.ExpandWidth(true));
            
            DrawCheckbox("Auto Process on Scene Objects", ref bindingSetting.autoProcessSceneObjects);

            if (bindingSetting.autoProcessSceneObjects)
            {
                DrawCheckbox("Ignore Game Components", ref bindingSetting.ignoreGameComponent);
            }

            EditorGUILayout.Space();
            
            DrawList("Default Setting", "", bindingSetting.defaultSettings,
                data =>
                {
                    ...
                    DrawButton("X", () => { bindingSetting.defaultSettings.Remove(data); },
                        GUILayout.MaxWidth(17),
                        GUILayout.ExpandWidth(false));

                    DrawButton("C", () => { bindingSetting.defaultSettings.Add(data.JsonClone()); },
